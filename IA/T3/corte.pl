%definição da profundidade a cortar
corteP(3).

g(Jogo):- [Jogo], estado_inicial(Ei), minimax_decidir(Ei,Op),
casa_pos(Ei,H,Op,1), write(H), nl.
...
% se a profundidade for igual ao corte, é o valor da função avaliação
%se o estado for terminal, a função aval retorna 4, o que seria identico ao peredicado terminal.
minimax_valor(Ei,Val,P):-  corteP(N), N=<P, aval(Ei,Val),!; terminal(Ei), valor(Ei, Val, P).
